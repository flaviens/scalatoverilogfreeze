...
module DebugTransportModuleJTAG(
  ...
  `ifndef SYNTHESIS
    wire [31:0] _tapIO_T =
      {io_jtag_version, io_jtag_part_number, io_jtag_mfr_id, 1'h1} % 32'h2;
    always @(posedge io_jtag_clock) begin
      if (~io_jtag_reset & _dmiAccessChain_io_update_valid & ~stickyBusyReg & ~_GEN_1
          & _GEN_0) begin
      ...
      if (~io_jtag_reset & _tapIO_T[1:0] != 2'h1) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: LSB must be set in IDCODE, see 12.1.1d\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~io_jtag_reset & io_jtag_mfr_id == 11'h7F) begin
        if (`ASSERT_VERBOSE_COND_)
  ...
  always @(posedge io_jtag_clock or posedge io_jtag_reset) begin
    if (io_jtag_reset) begin
      ...
      stickyBusyReg <=
        ~_GEN_2 & (_dmiAccessChain_io_capture_capture & _busy_T_1 | stickyBusyReg);
      ...
      dmiReqValidReg <=
        ...
        & (_dmiAccessChain_io_update_valid & ~(stickyBusyReg | _GEN_1) | dmiReqValidReg);
    end
  ...
  always @(posedge io_jtag_clock) begin
    if (~_dmiAccessChain_io_update_valid | stickyBusyReg) begin
      ...
      dmiReqReg_addr <= _GEN_1 ? 7'h0 : _dmiAccessChain_io_update_bits_addr;
      dmiReqReg_data <= _GEN_1 ? 32'h0 : _dmiAccessChain_io_update_bits_data;
      dmiReqReg_op <= _GEN_1 ? 2'h0 : _dmiAccessChain_io_update_bits_op;
    end
  end // always @(posedge)
  ...
  CaptureChain tapIO_idcodeChain (
    ...
    .io_chainIn_update
      (tapIO_chainToSelect_0_2 & _tapIO_controllerInternal_io_dataChainOut_update),
