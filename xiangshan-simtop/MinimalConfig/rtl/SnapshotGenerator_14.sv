// Generated by CIRCT firtool-1.135.0

// Users can define 'STOP_COND' to add an extra gate to stop conditions.
`ifndef STOP_COND_
  `ifdef STOP_COND
    `define STOP_COND_ (`STOP_COND)
  `else  // STOP_COND
    `define STOP_COND_ 1
  `endif // STOP_COND
`endif // not def STOP_COND_

// Users can define 'ASSERT_VERBOSE_COND' to add an extra gate to assert error printing.
`ifndef ASSERT_VERBOSE_COND_
  `ifdef ASSERT_VERBOSE_COND
    `define ASSERT_VERBOSE_COND_ (`ASSERT_VERBOSE_COND)
  `else  // ASSERT_VERBOSE_COND
    `define ASSERT_VERBOSE_COND_ 1
  `endif // ASSERT_VERBOSE_COND
`endif // not def ASSERT_VERBOSE_COND_

// Include register initializers in init blocks unless synthesis is set
`ifndef RANDOMIZE
  `ifdef RANDOMIZE_REG_INIT
    `define RANDOMIZE
  `endif // RANDOMIZE_REG_INIT
`endif // not def RANDOMIZE
`ifndef SYNTHESIS
  `ifndef ENABLE_INITIAL_REG_
    `define ENABLE_INITIAL_REG_
  `endif // not def ENABLE_INITIAL_REG_
`endif // not def SYNTHESIS

// Standard header to adapt well known macros for register randomization.

// RANDOM may be set to an expression that produces a 32-bit random unsigned value.
`ifndef RANDOM
  `define RANDOM $random
`endif // not def RANDOM

// Users can define INIT_RANDOM as general code that gets injected into the
// initializer block for modules with registers.
`ifndef INIT_RANDOM
  `define INIT_RANDOM
`endif // not def INIT_RANDOM

// If using random initialization, you can also define RANDOMIZE_DELAY to
// customize the delay used, otherwise 0.002 is used.
`ifndef RANDOMIZE_DELAY
  `define RANDOMIZE_DELAY 0.002
`endif // not def RANDOMIZE_DELAY

// Define INIT_RANDOM_PROLOG_ for use in our modules below.
`ifndef INIT_RANDOM_PROLOG_
  `ifdef RANDOMIZE
    `ifdef VERILATOR
      `define INIT_RANDOM_PROLOG_ `INIT_RANDOM
    `else  // VERILATOR
      `define INIT_RANDOM_PROLOG_ `INIT_RANDOM #`RANDOMIZE_DELAY begin end
    `endif // VERILATOR
  `else  // RANDOMIZE
    `define INIT_RANDOM_PROLOG_
  `endif // RANDOMIZE
`endif // not def INIT_RANDOM_PROLOG_
module SnapshotGenerator_14(
  input        clock,
  input        reset,
  input        io_enq,
  input        io_enqData_robIdx_0_flag,
  input  [5:0] io_enqData_robIdx_0_value,
  input        io_enqData_robIdx_1_flag,
  input  [5:0] io_enqData_robIdx_1_value,
  input        io_enqData_robIdx_2_flag,
  input  [5:0] io_enqData_robIdx_2_value,
  input        io_enqData_robIdx_3_flag,
  input  [5:0] io_enqData_robIdx_3_value,
  input        io_enqData_robIdx_4_flag,
  input  [5:0] io_enqData_robIdx_4_value,
  input        io_enqData_robIdx_5_flag,
  input  [5:0] io_enqData_robIdx_5_value,
  input        io_enqData_robIdx_6_flag,
  input  [5:0] io_enqData_robIdx_6_value,
  input        io_enqData_robIdx_7_flag,
  input  [5:0] io_enqData_robIdx_7_value,
  input        io_enqData_isCFI_0,
  input        io_enqData_isCFI_1,
  input        io_enqData_isCFI_2,
  input        io_enqData_isCFI_3,
  input        io_enqData_isCFI_4,
  input        io_enqData_isCFI_5,
  input        io_enqData_isCFI_6,
  input        io_enqData_isCFI_7,
  input        io_deq,
  input        io_redirect,
  input        io_flushVec_0,
  input        io_flushVec_1,
  input        io_flushVec_2,
  input        io_flushVec_3,
  output       io_snapshots_0_robIdx_0_flag,
  output [5:0] io_snapshots_0_robIdx_0_value,
  output       io_snapshots_0_robIdx_1_flag,
  output [5:0] io_snapshots_0_robIdx_1_value,
  output       io_snapshots_0_robIdx_2_flag,
  output [5:0] io_snapshots_0_robIdx_2_value,
  output       io_snapshots_0_robIdx_3_flag,
  output [5:0] io_snapshots_0_robIdx_3_value,
  output       io_snapshots_0_robIdx_4_flag,
  output [5:0] io_snapshots_0_robIdx_4_value,
  output       io_snapshots_0_robIdx_5_flag,
  output [5:0] io_snapshots_0_robIdx_5_value,
  output       io_snapshots_0_robIdx_6_flag,
  output [5:0] io_snapshots_0_robIdx_6_value,
  output       io_snapshots_0_robIdx_7_flag,
  output [5:0] io_snapshots_0_robIdx_7_value,
  output       io_snapshots_0_isCFI_0,
  output       io_snapshots_0_isCFI_1,
  output       io_snapshots_0_isCFI_2,
  output       io_snapshots_0_isCFI_3,
  output       io_snapshots_0_isCFI_4,
  output       io_snapshots_0_isCFI_5,
  output       io_snapshots_0_isCFI_6,
  output       io_snapshots_0_isCFI_7,
  output       io_snapshots_1_robIdx_0_flag,
  output [5:0] io_snapshots_1_robIdx_0_value,
  output       io_snapshots_1_robIdx_1_flag,
  output [5:0] io_snapshots_1_robIdx_1_value,
  output       io_snapshots_1_robIdx_2_flag,
  output [5:0] io_snapshots_1_robIdx_2_value,
  output       io_snapshots_1_robIdx_3_flag,
  output [5:0] io_snapshots_1_robIdx_3_value,
  output       io_snapshots_1_robIdx_4_flag,
  output [5:0] io_snapshots_1_robIdx_4_value,
  output       io_snapshots_1_robIdx_5_flag,
  output [5:0] io_snapshots_1_robIdx_5_value,
  output       io_snapshots_1_robIdx_6_flag,
  output [5:0] io_snapshots_1_robIdx_6_value,
  output       io_snapshots_1_robIdx_7_flag,
  output [5:0] io_snapshots_1_robIdx_7_value,
  output       io_snapshots_1_isCFI_0,
  output       io_snapshots_1_isCFI_1,
  output       io_snapshots_1_isCFI_2,
  output       io_snapshots_1_isCFI_3,
  output       io_snapshots_1_isCFI_4,
  output       io_snapshots_1_isCFI_5,
  output       io_snapshots_1_isCFI_6,
  output       io_snapshots_1_isCFI_7,
  output       io_snapshots_2_robIdx_0_flag,
  output [5:0] io_snapshots_2_robIdx_0_value,
  output       io_snapshots_2_robIdx_1_flag,
  output [5:0] io_snapshots_2_robIdx_1_value,
  output       io_snapshots_2_robIdx_2_flag,
  output [5:0] io_snapshots_2_robIdx_2_value,
  output       io_snapshots_2_robIdx_3_flag,
  output [5:0] io_snapshots_2_robIdx_3_value,
  output       io_snapshots_2_robIdx_4_flag,
  output [5:0] io_snapshots_2_robIdx_4_value,
  output       io_snapshots_2_robIdx_5_flag,
  output [5:0] io_snapshots_2_robIdx_5_value,
  output       io_snapshots_2_robIdx_6_flag,
  output [5:0] io_snapshots_2_robIdx_6_value,
  output       io_snapshots_2_robIdx_7_flag,
  output [5:0] io_snapshots_2_robIdx_7_value,
  output       io_snapshots_2_isCFI_0,
  output       io_snapshots_2_isCFI_1,
  output       io_snapshots_2_isCFI_2,
  output       io_snapshots_2_isCFI_3,
  output       io_snapshots_2_isCFI_4,
  output       io_snapshots_2_isCFI_5,
  output       io_snapshots_2_isCFI_6,
  output       io_snapshots_2_isCFI_7,
  output       io_snapshots_3_robIdx_0_flag,
  output [5:0] io_snapshots_3_robIdx_0_value,
  output       io_snapshots_3_robIdx_1_flag,
  output [5:0] io_snapshots_3_robIdx_1_value,
  output       io_snapshots_3_robIdx_2_flag,
  output [5:0] io_snapshots_3_robIdx_2_value,
  output       io_snapshots_3_robIdx_3_flag,
  output [5:0] io_snapshots_3_robIdx_3_value,
  output       io_snapshots_3_robIdx_4_flag,
  output [5:0] io_snapshots_3_robIdx_4_value,
  output       io_snapshots_3_robIdx_5_flag,
  output [5:0] io_snapshots_3_robIdx_5_value,
  output       io_snapshots_3_robIdx_6_flag,
  output [5:0] io_snapshots_3_robIdx_6_value,
  output       io_snapshots_3_robIdx_7_flag,
  output [5:0] io_snapshots_3_robIdx_7_value,
  output       io_snapshots_3_isCFI_0,
  output       io_snapshots_3_isCFI_1,
  output       io_snapshots_3_isCFI_2,
  output       io_snapshots_3_isCFI_3,
  output       io_snapshots_3_isCFI_4,
  output       io_snapshots_3_isCFI_5,
  output       io_snapshots_3_isCFI_6,
  output       io_snapshots_3_isCFI_7,
  output       io_enqPtr_flag,
  output [1:0] io_enqPtr_value,
  output       io_deqPtr_flag,
  output [1:0] io_deqPtr_value,
  output       io_valids_0,
  output       io_valids_1,
  output       io_valids_2,
  output       io_valids_3
);

  reg        snapshots_0_robIdx_0_flag;
  reg  [5:0] snapshots_0_robIdx_0_value;
  reg        snapshots_0_robIdx_1_flag;
  reg  [5:0] snapshots_0_robIdx_1_value;
  reg        snapshots_0_robIdx_2_flag;
  reg  [5:0] snapshots_0_robIdx_2_value;
  reg        snapshots_0_robIdx_3_flag;
  reg  [5:0] snapshots_0_robIdx_3_value;
  reg        snapshots_0_robIdx_4_flag;
  reg  [5:0] snapshots_0_robIdx_4_value;
  reg        snapshots_0_robIdx_5_flag;
  reg  [5:0] snapshots_0_robIdx_5_value;
  reg        snapshots_0_robIdx_6_flag;
  reg  [5:0] snapshots_0_robIdx_6_value;
  reg        snapshots_0_robIdx_7_flag;
  reg  [5:0] snapshots_0_robIdx_7_value;
  reg        snapshots_0_isCFI_0;
  reg        snapshots_0_isCFI_1;
  reg        snapshots_0_isCFI_2;
  reg        snapshots_0_isCFI_3;
  reg        snapshots_0_isCFI_4;
  reg        snapshots_0_isCFI_5;
  reg        snapshots_0_isCFI_6;
  reg        snapshots_0_isCFI_7;
  reg        snapshots_1_robIdx_0_flag;
  reg  [5:0] snapshots_1_robIdx_0_value;
  reg        snapshots_1_robIdx_1_flag;
  reg  [5:0] snapshots_1_robIdx_1_value;
  reg        snapshots_1_robIdx_2_flag;
  reg  [5:0] snapshots_1_robIdx_2_value;
  reg        snapshots_1_robIdx_3_flag;
  reg  [5:0] snapshots_1_robIdx_3_value;
  reg        snapshots_1_robIdx_4_flag;
  reg  [5:0] snapshots_1_robIdx_4_value;
  reg        snapshots_1_robIdx_5_flag;
  reg  [5:0] snapshots_1_robIdx_5_value;
  reg        snapshots_1_robIdx_6_flag;
  reg  [5:0] snapshots_1_robIdx_6_value;
  reg        snapshots_1_robIdx_7_flag;
  reg  [5:0] snapshots_1_robIdx_7_value;
  reg        snapshots_1_isCFI_0;
  reg        snapshots_1_isCFI_1;
  reg        snapshots_1_isCFI_2;
  reg        snapshots_1_isCFI_3;
  reg        snapshots_1_isCFI_4;
  reg        snapshots_1_isCFI_5;
  reg        snapshots_1_isCFI_6;
  reg        snapshots_1_isCFI_7;
  reg        snapshots_2_robIdx_0_flag;
  reg  [5:0] snapshots_2_robIdx_0_value;
  reg        snapshots_2_robIdx_1_flag;
  reg  [5:0] snapshots_2_robIdx_1_value;
  reg        snapshots_2_robIdx_2_flag;
  reg  [5:0] snapshots_2_robIdx_2_value;
  reg        snapshots_2_robIdx_3_flag;
  reg  [5:0] snapshots_2_robIdx_3_value;
  reg        snapshots_2_robIdx_4_flag;
  reg  [5:0] snapshots_2_robIdx_4_value;
  reg        snapshots_2_robIdx_5_flag;
  reg  [5:0] snapshots_2_robIdx_5_value;
  reg        snapshots_2_robIdx_6_flag;
  reg  [5:0] snapshots_2_robIdx_6_value;
  reg        snapshots_2_robIdx_7_flag;
  reg  [5:0] snapshots_2_robIdx_7_value;
  reg        snapshots_2_isCFI_0;
  reg        snapshots_2_isCFI_1;
  reg        snapshots_2_isCFI_2;
  reg        snapshots_2_isCFI_3;
  reg        snapshots_2_isCFI_4;
  reg        snapshots_2_isCFI_5;
  reg        snapshots_2_isCFI_6;
  reg        snapshots_2_isCFI_7;
  reg        snapshots_3_robIdx_0_flag;
  reg  [5:0] snapshots_3_robIdx_0_value;
  reg        snapshots_3_robIdx_1_flag;
  reg  [5:0] snapshots_3_robIdx_1_value;
  reg        snapshots_3_robIdx_2_flag;
  reg  [5:0] snapshots_3_robIdx_2_value;
  reg        snapshots_3_robIdx_3_flag;
  reg  [5:0] snapshots_3_robIdx_3_value;
  reg        snapshots_3_robIdx_4_flag;
  reg  [5:0] snapshots_3_robIdx_4_value;
  reg        snapshots_3_robIdx_5_flag;
  reg  [5:0] snapshots_3_robIdx_5_value;
  reg        snapshots_3_robIdx_6_flag;
  reg  [5:0] snapshots_3_robIdx_6_value;
  reg        snapshots_3_robIdx_7_flag;
  reg  [5:0] snapshots_3_robIdx_7_value;
  reg        snapshots_3_isCFI_0;
  reg        snapshots_3_isCFI_1;
  reg        snapshots_3_isCFI_2;
  reg        snapshots_3_isCFI_3;
  reg        snapshots_3_isCFI_4;
  reg        snapshots_3_isCFI_5;
  reg        snapshots_3_isCFI_6;
  reg        snapshots_3_isCFI_7;
  reg        snptEnqPtr_flag;
  reg  [1:0] snptEnqPtr_value;
  reg        snptDeqPtr_flag;
  reg  [1:0] snptDeqPtr_value;
  reg        snptValids_0;
  reg        snptValids_1;
  reg        snptValids_2;
  reg        snptValids_3;
  wire       _GEN =
    ~io_redirect
    & ~(snptEnqPtr_flag != snptDeqPtr_flag & snptEnqPtr_value == snptDeqPtr_value)
    & io_enq;
  wire       _GEN_0 = _GEN & snptEnqPtr_value == 2'h0;
  wire       _GEN_1 = _GEN & snptEnqPtr_value == 2'h1;
  wire       _GEN_2 = _GEN & snptEnqPtr_value == 2'h2;
  wire       _GEN_3 = _GEN & (&snptEnqPtr_value);
  wire [2:0] _GEN_4 = {snptEnqPtr_flag, snptEnqPtr_value};
  wire       _GEN_5 = ~io_redirect & io_deq;
  wire       _GEN_6 = _GEN_5 & _GEN_4 == {snptDeqPtr_flag, snptDeqPtr_value};
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (_GEN_6 & ~reset) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed at LogUtils.scala:133\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  `endif // not def SYNTHESIS
  always @(posedge clock) begin
    if (_GEN_0) begin
      snapshots_0_robIdx_0_flag <= io_enqData_robIdx_0_flag;
      snapshots_0_robIdx_0_value <= io_enqData_robIdx_0_value;
      snapshots_0_robIdx_1_flag <= io_enqData_robIdx_1_flag;
      snapshots_0_robIdx_1_value <= io_enqData_robIdx_1_value;
      snapshots_0_robIdx_2_flag <= io_enqData_robIdx_2_flag;
      snapshots_0_robIdx_2_value <= io_enqData_robIdx_2_value;
      snapshots_0_robIdx_3_flag <= io_enqData_robIdx_3_flag;
      snapshots_0_robIdx_3_value <= io_enqData_robIdx_3_value;
      snapshots_0_robIdx_4_flag <= io_enqData_robIdx_4_flag;
      snapshots_0_robIdx_4_value <= io_enqData_robIdx_4_value;
      snapshots_0_robIdx_5_flag <= io_enqData_robIdx_5_flag;
      snapshots_0_robIdx_5_value <= io_enqData_robIdx_5_value;
      snapshots_0_robIdx_6_flag <= io_enqData_robIdx_6_flag;
      snapshots_0_robIdx_6_value <= io_enqData_robIdx_6_value;
      snapshots_0_robIdx_7_flag <= io_enqData_robIdx_7_flag;
      snapshots_0_robIdx_7_value <= io_enqData_robIdx_7_value;
      snapshots_0_isCFI_0 <= io_enqData_isCFI_0;
      snapshots_0_isCFI_1 <= io_enqData_isCFI_1;
      snapshots_0_isCFI_2 <= io_enqData_isCFI_2;
      snapshots_0_isCFI_3 <= io_enqData_isCFI_3;
      snapshots_0_isCFI_4 <= io_enqData_isCFI_4;
      snapshots_0_isCFI_5 <= io_enqData_isCFI_5;
      snapshots_0_isCFI_6 <= io_enqData_isCFI_6;
      snapshots_0_isCFI_7 <= io_enqData_isCFI_7;
    end
    if (_GEN_1) begin
      snapshots_1_robIdx_0_flag <= io_enqData_robIdx_0_flag;
      snapshots_1_robIdx_0_value <= io_enqData_robIdx_0_value;
      snapshots_1_robIdx_1_flag <= io_enqData_robIdx_1_flag;
      snapshots_1_robIdx_1_value <= io_enqData_robIdx_1_value;
      snapshots_1_robIdx_2_flag <= io_enqData_robIdx_2_flag;
      snapshots_1_robIdx_2_value <= io_enqData_robIdx_2_value;
      snapshots_1_robIdx_3_flag <= io_enqData_robIdx_3_flag;
      snapshots_1_robIdx_3_value <= io_enqData_robIdx_3_value;
      snapshots_1_robIdx_4_flag <= io_enqData_robIdx_4_flag;
      snapshots_1_robIdx_4_value <= io_enqData_robIdx_4_value;
      snapshots_1_robIdx_5_flag <= io_enqData_robIdx_5_flag;
      snapshots_1_robIdx_5_value <= io_enqData_robIdx_5_value;
      snapshots_1_robIdx_6_flag <= io_enqData_robIdx_6_flag;
      snapshots_1_robIdx_6_value <= io_enqData_robIdx_6_value;
      snapshots_1_robIdx_7_flag <= io_enqData_robIdx_7_flag;
      snapshots_1_robIdx_7_value <= io_enqData_robIdx_7_value;
      snapshots_1_isCFI_0 <= io_enqData_isCFI_0;
      snapshots_1_isCFI_1 <= io_enqData_isCFI_1;
      snapshots_1_isCFI_2 <= io_enqData_isCFI_2;
      snapshots_1_isCFI_3 <= io_enqData_isCFI_3;
      snapshots_1_isCFI_4 <= io_enqData_isCFI_4;
      snapshots_1_isCFI_5 <= io_enqData_isCFI_5;
      snapshots_1_isCFI_6 <= io_enqData_isCFI_6;
      snapshots_1_isCFI_7 <= io_enqData_isCFI_7;
    end
    if (_GEN_2) begin
      snapshots_2_robIdx_0_flag <= io_enqData_robIdx_0_flag;
      snapshots_2_robIdx_0_value <= io_enqData_robIdx_0_value;
      snapshots_2_robIdx_1_flag <= io_enqData_robIdx_1_flag;
      snapshots_2_robIdx_1_value <= io_enqData_robIdx_1_value;
      snapshots_2_robIdx_2_flag <= io_enqData_robIdx_2_flag;
      snapshots_2_robIdx_2_value <= io_enqData_robIdx_2_value;
      snapshots_2_robIdx_3_flag <= io_enqData_robIdx_3_flag;
      snapshots_2_robIdx_3_value <= io_enqData_robIdx_3_value;
      snapshots_2_robIdx_4_flag <= io_enqData_robIdx_4_flag;
      snapshots_2_robIdx_4_value <= io_enqData_robIdx_4_value;
      snapshots_2_robIdx_5_flag <= io_enqData_robIdx_5_flag;
      snapshots_2_robIdx_5_value <= io_enqData_robIdx_5_value;
      snapshots_2_robIdx_6_flag <= io_enqData_robIdx_6_flag;
      snapshots_2_robIdx_6_value <= io_enqData_robIdx_6_value;
      snapshots_2_robIdx_7_flag <= io_enqData_robIdx_7_flag;
      snapshots_2_robIdx_7_value <= io_enqData_robIdx_7_value;
      snapshots_2_isCFI_0 <= io_enqData_isCFI_0;
      snapshots_2_isCFI_1 <= io_enqData_isCFI_1;
      snapshots_2_isCFI_2 <= io_enqData_isCFI_2;
      snapshots_2_isCFI_3 <= io_enqData_isCFI_3;
      snapshots_2_isCFI_4 <= io_enqData_isCFI_4;
      snapshots_2_isCFI_5 <= io_enqData_isCFI_5;
      snapshots_2_isCFI_6 <= io_enqData_isCFI_6;
      snapshots_2_isCFI_7 <= io_enqData_isCFI_7;
    end
    if (_GEN_3) begin
      snapshots_3_robIdx_0_flag <= io_enqData_robIdx_0_flag;
      snapshots_3_robIdx_0_value <= io_enqData_robIdx_0_value;
      snapshots_3_robIdx_1_flag <= io_enqData_robIdx_1_flag;
      snapshots_3_robIdx_1_value <= io_enqData_robIdx_1_value;
      snapshots_3_robIdx_2_flag <= io_enqData_robIdx_2_flag;
      snapshots_3_robIdx_2_value <= io_enqData_robIdx_2_value;
      snapshots_3_robIdx_3_flag <= io_enqData_robIdx_3_flag;
      snapshots_3_robIdx_3_value <= io_enqData_robIdx_3_value;
      snapshots_3_robIdx_4_flag <= io_enqData_robIdx_4_flag;
      snapshots_3_robIdx_4_value <= io_enqData_robIdx_4_value;
      snapshots_3_robIdx_5_flag <= io_enqData_robIdx_5_flag;
      snapshots_3_robIdx_5_value <= io_enqData_robIdx_5_value;
      snapshots_3_robIdx_6_flag <= io_enqData_robIdx_6_flag;
      snapshots_3_robIdx_6_value <= io_enqData_robIdx_6_value;
      snapshots_3_robIdx_7_flag <= io_enqData_robIdx_7_flag;
      snapshots_3_robIdx_7_value <= io_enqData_robIdx_7_value;
      snapshots_3_isCFI_0 <= io_enqData_isCFI_0;
      snapshots_3_isCFI_1 <= io_enqData_isCFI_1;
      snapshots_3_isCFI_2 <= io_enqData_isCFI_2;
      snapshots_3_isCFI_3 <= io_enqData_isCFI_3;
      snapshots_3_isCFI_4 <= io_enqData_isCFI_4;
      snapshots_3_isCFI_5 <= io_enqData_isCFI_5;
      snapshots_3_isCFI_6 <= io_enqData_isCFI_6;
      snapshots_3_isCFI_7 <= io_enqData_isCFI_7;
    end
  end // always @(posedge)
  wire [3:0] _GEN_7 = {{snptValids_3}, {snptValids_2}, {snptValids_1}, {snptValids_0}};
  wire [3:0] _GEN_8 =
    {{io_flushVec_3}, {io_flushVec_2}, {io_flushVec_1}, {io_flushVec_0}};
  wire       newEnqPtrQualified_0 = ~_GEN_7[snptDeqPtr_value] | _GEN_8[snptDeqPtr_value];
  wire [2:0] _snptEnqPtr_new_ptr_T_1 = 3'(_GEN_4 + 3'h1);
  wire [2:0] _GEN_9 = {snptDeqPtr_flag, snptDeqPtr_value};
  wire [2:0] _newEnqPtrCandidate_new_ptr_T_7 = 3'(_GEN_9 + 3'h1);
  wire [2:0] _newEnqPtrCandidate_new_ptr_T_13 = 3'(_GEN_9 + 3'h2);
  wire [2:0] _newEnqPtrCandidate_new_ptr_T_19 = 3'(_GEN_9 + 3'h3);
  wire       _GEN_10 = _GEN_7[_newEnqPtrCandidate_new_ptr_T_7[1:0]];
  wire       newEnqPtrQualified_1 =
    _GEN_10 & (~_GEN_10 | _GEN_8[_newEnqPtrCandidate_new_ptr_T_7[1:0]]);
  wire       _GEN_11 = _GEN_7[_newEnqPtrCandidate_new_ptr_T_13[1:0]];
  wire       newEnqPtrQualified_2 =
    _GEN_11 & (~_GEN_11 | _GEN_8[_newEnqPtrCandidate_new_ptr_T_13[1:0]]);
  wire [2:0] _snptDeqPtr_new_ptr_T_1 = 3'(_GEN_9 + 3'h1);
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      snptEnqPtr_flag <= 1'h0;
      snptEnqPtr_value <= 2'h0;
      snptDeqPtr_flag <= 1'h0;
      snptDeqPtr_value <= 2'h0;
      snptValids_0 <= 1'h0;
      snptValids_1 <= 1'h0;
      snptValids_2 <= 1'h0;
      snptValids_3 <= 1'h0;
    end
    else begin
      if (|({io_flushVec_0, io_flushVec_1, io_flushVec_2, io_flushVec_3}
            & {snptValids_0, snptValids_1, snptValids_2, snptValids_3})) begin
        snptEnqPtr_flag <=
          newEnqPtrQualified_0
            ? snptDeqPtr_flag
            : newEnqPtrQualified_1
                ? _newEnqPtrCandidate_new_ptr_T_7[2]
                : newEnqPtrQualified_2
                    ? _newEnqPtrCandidate_new_ptr_T_13[2]
                    : _newEnqPtrCandidate_new_ptr_T_19[2];
        snptEnqPtr_value <=
          newEnqPtrQualified_0
            ? snptDeqPtr_value
            : newEnqPtrQualified_1
                ? _newEnqPtrCandidate_new_ptr_T_7[1:0]
                : newEnqPtrQualified_2
                    ? _newEnqPtrCandidate_new_ptr_T_13[1:0]
                    : _newEnqPtrCandidate_new_ptr_T_19[1:0];
      end
      else if (_GEN) begin
        snptEnqPtr_flag <= _snptEnqPtr_new_ptr_T_1[2];
        snptEnqPtr_value <= _snptEnqPtr_new_ptr_T_1[1:0];
      end
      if (_GEN_5) begin
        snptDeqPtr_flag <= _snptDeqPtr_new_ptr_T_1[2];
        snptDeqPtr_value <= _snptDeqPtr_new_ptr_T_1[1:0];
      end
      snptValids_0 <=
        ~(io_flushVec_0 | _GEN_5 & snptDeqPtr_value == 2'h0) & (_GEN_0 | snptValids_0);
      snptValids_1 <=
        ~(io_flushVec_1 | _GEN_5 & snptDeqPtr_value == 2'h1) & (_GEN_1 | snptValids_1);
      snptValids_2 <=
        ~(io_flushVec_2 | _GEN_5 & snptDeqPtr_value == 2'h2) & (_GEN_2 | snptValids_2);
      snptValids_3 <=
        ~(io_flushVec_3 | _GEN_5 & (&snptDeqPtr_value)) & (_GEN_3 | snptValids_3);
    end
  end // always @(posedge, posedge)
  `ifdef ENABLE_INITIAL_REG_
    `ifdef FIRRTL_BEFORE_INITIAL
      `FIRRTL_BEFORE_INITIAL
    `endif // FIRRTL_BEFORE_INITIAL
    logic [31:0] _RANDOM[0:8];
    initial begin
      `ifdef INIT_RANDOM_PROLOG_
        `INIT_RANDOM_PROLOG_
      `endif // INIT_RANDOM_PROLOG_
      `ifdef RANDOMIZE_REG_INIT
        for (logic [3:0] i = 4'h0; i < 4'h9; i += 4'h1) begin
          _RANDOM[i] = `RANDOM;
        end
        snapshots_0_robIdx_0_flag = _RANDOM[4'h0][0];
        snapshots_0_robIdx_0_value = _RANDOM[4'h0][6:1];
        snapshots_0_robIdx_1_flag = _RANDOM[4'h0][7];
        snapshots_0_robIdx_1_value = _RANDOM[4'h0][13:8];
        snapshots_0_robIdx_2_flag = _RANDOM[4'h0][14];
        snapshots_0_robIdx_2_value = _RANDOM[4'h0][20:15];
        snapshots_0_robIdx_3_flag = _RANDOM[4'h0][21];
        snapshots_0_robIdx_3_value = _RANDOM[4'h0][27:22];
        snapshots_0_robIdx_4_flag = _RANDOM[4'h0][28];
        snapshots_0_robIdx_4_value = {_RANDOM[4'h0][31:29], _RANDOM[4'h1][2:0]};
        snapshots_0_robIdx_5_flag = _RANDOM[4'h1][3];
        snapshots_0_robIdx_5_value = _RANDOM[4'h1][9:4];
        snapshots_0_robIdx_6_flag = _RANDOM[4'h1][10];
        snapshots_0_robIdx_6_value = _RANDOM[4'h1][16:11];
        snapshots_0_robIdx_7_flag = _RANDOM[4'h1][17];
        snapshots_0_robIdx_7_value = _RANDOM[4'h1][23:18];
        snapshots_0_isCFI_0 = _RANDOM[4'h1][24];
        snapshots_0_isCFI_1 = _RANDOM[4'h1][25];
        snapshots_0_isCFI_2 = _RANDOM[4'h1][26];
        snapshots_0_isCFI_3 = _RANDOM[4'h1][27];
        snapshots_0_isCFI_4 = _RANDOM[4'h1][28];
        snapshots_0_isCFI_5 = _RANDOM[4'h1][29];
        snapshots_0_isCFI_6 = _RANDOM[4'h1][30];
        snapshots_0_isCFI_7 = _RANDOM[4'h1][31];
        snapshots_1_robIdx_0_flag = _RANDOM[4'h2][0];
        snapshots_1_robIdx_0_value = _RANDOM[4'h2][6:1];
        snapshots_1_robIdx_1_flag = _RANDOM[4'h2][7];
        snapshots_1_robIdx_1_value = _RANDOM[4'h2][13:8];
        snapshots_1_robIdx_2_flag = _RANDOM[4'h2][14];
        snapshots_1_robIdx_2_value = _RANDOM[4'h2][20:15];
        snapshots_1_robIdx_3_flag = _RANDOM[4'h2][21];
        snapshots_1_robIdx_3_value = _RANDOM[4'h2][27:22];
        snapshots_1_robIdx_4_flag = _RANDOM[4'h2][28];
        snapshots_1_robIdx_4_value = {_RANDOM[4'h2][31:29], _RANDOM[4'h3][2:0]};
        snapshots_1_robIdx_5_flag = _RANDOM[4'h3][3];
        snapshots_1_robIdx_5_value = _RANDOM[4'h3][9:4];
        snapshots_1_robIdx_6_flag = _RANDOM[4'h3][10];
        snapshots_1_robIdx_6_value = _RANDOM[4'h3][16:11];
        snapshots_1_robIdx_7_flag = _RANDOM[4'h3][17];
        snapshots_1_robIdx_7_value = _RANDOM[4'h3][23:18];
        snapshots_1_isCFI_0 = _RANDOM[4'h3][24];
        snapshots_1_isCFI_1 = _RANDOM[4'h3][25];
        snapshots_1_isCFI_2 = _RANDOM[4'h3][26];
        snapshots_1_isCFI_3 = _RANDOM[4'h3][27];
        snapshots_1_isCFI_4 = _RANDOM[4'h3][28];
        snapshots_1_isCFI_5 = _RANDOM[4'h3][29];
        snapshots_1_isCFI_6 = _RANDOM[4'h3][30];
        snapshots_1_isCFI_7 = _RANDOM[4'h3][31];
        snapshots_2_robIdx_0_flag = _RANDOM[4'h4][0];
        snapshots_2_robIdx_0_value = _RANDOM[4'h4][6:1];
        snapshots_2_robIdx_1_flag = _RANDOM[4'h4][7];
        snapshots_2_robIdx_1_value = _RANDOM[4'h4][13:8];
        snapshots_2_robIdx_2_flag = _RANDOM[4'h4][14];
        snapshots_2_robIdx_2_value = _RANDOM[4'h4][20:15];
        snapshots_2_robIdx_3_flag = _RANDOM[4'h4][21];
        snapshots_2_robIdx_3_value = _RANDOM[4'h4][27:22];
        snapshots_2_robIdx_4_flag = _RANDOM[4'h4][28];
        snapshots_2_robIdx_4_value = {_RANDOM[4'h4][31:29], _RANDOM[4'h5][2:0]};
        snapshots_2_robIdx_5_flag = _RANDOM[4'h5][3];
        snapshots_2_robIdx_5_value = _RANDOM[4'h5][9:4];
        snapshots_2_robIdx_6_flag = _RANDOM[4'h5][10];
        snapshots_2_robIdx_6_value = _RANDOM[4'h5][16:11];
        snapshots_2_robIdx_7_flag = _RANDOM[4'h5][17];
        snapshots_2_robIdx_7_value = _RANDOM[4'h5][23:18];
        snapshots_2_isCFI_0 = _RANDOM[4'h5][24];
        snapshots_2_isCFI_1 = _RANDOM[4'h5][25];
        snapshots_2_isCFI_2 = _RANDOM[4'h5][26];
        snapshots_2_isCFI_3 = _RANDOM[4'h5][27];
        snapshots_2_isCFI_4 = _RANDOM[4'h5][28];
        snapshots_2_isCFI_5 = _RANDOM[4'h5][29];
        snapshots_2_isCFI_6 = _RANDOM[4'h5][30];
        snapshots_2_isCFI_7 = _RANDOM[4'h5][31];
        snapshots_3_robIdx_0_flag = _RANDOM[4'h6][0];
        snapshots_3_robIdx_0_value = _RANDOM[4'h6][6:1];
        snapshots_3_robIdx_1_flag = _RANDOM[4'h6][7];
        snapshots_3_robIdx_1_value = _RANDOM[4'h6][13:8];
        snapshots_3_robIdx_2_flag = _RANDOM[4'h6][14];
        snapshots_3_robIdx_2_value = _RANDOM[4'h6][20:15];
        snapshots_3_robIdx_3_flag = _RANDOM[4'h6][21];
        snapshots_3_robIdx_3_value = _RANDOM[4'h6][27:22];
        snapshots_3_robIdx_4_flag = _RANDOM[4'h6][28];
        snapshots_3_robIdx_4_value = {_RANDOM[4'h6][31:29], _RANDOM[4'h7][2:0]};
        snapshots_3_robIdx_5_flag = _RANDOM[4'h7][3];
        snapshots_3_robIdx_5_value = _RANDOM[4'h7][9:4];
        snapshots_3_robIdx_6_flag = _RANDOM[4'h7][10];
        snapshots_3_robIdx_6_value = _RANDOM[4'h7][16:11];
        snapshots_3_robIdx_7_flag = _RANDOM[4'h7][17];
        snapshots_3_robIdx_7_value = _RANDOM[4'h7][23:18];
        snapshots_3_isCFI_0 = _RANDOM[4'h7][24];
        snapshots_3_isCFI_1 = _RANDOM[4'h7][25];
        snapshots_3_isCFI_2 = _RANDOM[4'h7][26];
        snapshots_3_isCFI_3 = _RANDOM[4'h7][27];
        snapshots_3_isCFI_4 = _RANDOM[4'h7][28];
        snapshots_3_isCFI_5 = _RANDOM[4'h7][29];
        snapshots_3_isCFI_6 = _RANDOM[4'h7][30];
        snapshots_3_isCFI_7 = _RANDOM[4'h7][31];
        snptEnqPtr_flag = _RANDOM[4'h8][0];
        snptEnqPtr_value = _RANDOM[4'h8][2:1];
        snptDeqPtr_flag = _RANDOM[4'h8][3];
        snptDeqPtr_value = _RANDOM[4'h8][5:4];
        snptValids_0 = _RANDOM[4'h8][6];
        snptValids_1 = _RANDOM[4'h8][7];
        snptValids_2 = _RANDOM[4'h8][8];
        snptValids_3 = _RANDOM[4'h8][9];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        snptEnqPtr_flag = 1'h0;
        snptEnqPtr_value = 2'h0;
        snptDeqPtr_flag = 1'h0;
        snptDeqPtr_value = 2'h0;
        snptValids_0 = 1'h0;
        snptValids_1 = 1'h0;
        snptValids_2 = 1'h0;
        snptValids_3 = 1'h0;
      end
    end // initial
    `ifdef FIRRTL_AFTER_INITIAL
      `FIRRTL_AFTER_INITIAL
    `endif // FIRRTL_AFTER_INITIAL
  `endif // ENABLE_INITIAL_REG_
  assign io_snapshots_0_robIdx_0_flag = snapshots_0_robIdx_0_flag;
  assign io_snapshots_0_robIdx_0_value = snapshots_0_robIdx_0_value;
  assign io_snapshots_0_robIdx_1_flag = snapshots_0_robIdx_1_flag;
  assign io_snapshots_0_robIdx_1_value = snapshots_0_robIdx_1_value;
  assign io_snapshots_0_robIdx_2_flag = snapshots_0_robIdx_2_flag;
  assign io_snapshots_0_robIdx_2_value = snapshots_0_robIdx_2_value;
  assign io_snapshots_0_robIdx_3_flag = snapshots_0_robIdx_3_flag;
  assign io_snapshots_0_robIdx_3_value = snapshots_0_robIdx_3_value;
  assign io_snapshots_0_robIdx_4_flag = snapshots_0_robIdx_4_flag;
  assign io_snapshots_0_robIdx_4_value = snapshots_0_robIdx_4_value;
  assign io_snapshots_0_robIdx_5_flag = snapshots_0_robIdx_5_flag;
  assign io_snapshots_0_robIdx_5_value = snapshots_0_robIdx_5_value;
  assign io_snapshots_0_robIdx_6_flag = snapshots_0_robIdx_6_flag;
  assign io_snapshots_0_robIdx_6_value = snapshots_0_robIdx_6_value;
  assign io_snapshots_0_robIdx_7_flag = snapshots_0_robIdx_7_flag;
  assign io_snapshots_0_robIdx_7_value = snapshots_0_robIdx_7_value;
  assign io_snapshots_0_isCFI_0 = snapshots_0_isCFI_0;
  assign io_snapshots_0_isCFI_1 = snapshots_0_isCFI_1;
  assign io_snapshots_0_isCFI_2 = snapshots_0_isCFI_2;
  assign io_snapshots_0_isCFI_3 = snapshots_0_isCFI_3;
  assign io_snapshots_0_isCFI_4 = snapshots_0_isCFI_4;
  assign io_snapshots_0_isCFI_5 = snapshots_0_isCFI_5;
  assign io_snapshots_0_isCFI_6 = snapshots_0_isCFI_6;
  assign io_snapshots_0_isCFI_7 = snapshots_0_isCFI_7;
  assign io_snapshots_1_robIdx_0_flag = snapshots_1_robIdx_0_flag;
  assign io_snapshots_1_robIdx_0_value = snapshots_1_robIdx_0_value;
  assign io_snapshots_1_robIdx_1_flag = snapshots_1_robIdx_1_flag;
  assign io_snapshots_1_robIdx_1_value = snapshots_1_robIdx_1_value;
  assign io_snapshots_1_robIdx_2_flag = snapshots_1_robIdx_2_flag;
  assign io_snapshots_1_robIdx_2_value = snapshots_1_robIdx_2_value;
  assign io_snapshots_1_robIdx_3_flag = snapshots_1_robIdx_3_flag;
  assign io_snapshots_1_robIdx_3_value = snapshots_1_robIdx_3_value;
  assign io_snapshots_1_robIdx_4_flag = snapshots_1_robIdx_4_flag;
  assign io_snapshots_1_robIdx_4_value = snapshots_1_robIdx_4_value;
  assign io_snapshots_1_robIdx_5_flag = snapshots_1_robIdx_5_flag;
  assign io_snapshots_1_robIdx_5_value = snapshots_1_robIdx_5_value;
  assign io_snapshots_1_robIdx_6_flag = snapshots_1_robIdx_6_flag;
  assign io_snapshots_1_robIdx_6_value = snapshots_1_robIdx_6_value;
  assign io_snapshots_1_robIdx_7_flag = snapshots_1_robIdx_7_flag;
  assign io_snapshots_1_robIdx_7_value = snapshots_1_robIdx_7_value;
  assign io_snapshots_1_isCFI_0 = snapshots_1_isCFI_0;
  assign io_snapshots_1_isCFI_1 = snapshots_1_isCFI_1;
  assign io_snapshots_1_isCFI_2 = snapshots_1_isCFI_2;
  assign io_snapshots_1_isCFI_3 = snapshots_1_isCFI_3;
  assign io_snapshots_1_isCFI_4 = snapshots_1_isCFI_4;
  assign io_snapshots_1_isCFI_5 = snapshots_1_isCFI_5;
  assign io_snapshots_1_isCFI_6 = snapshots_1_isCFI_6;
  assign io_snapshots_1_isCFI_7 = snapshots_1_isCFI_7;
  assign io_snapshots_2_robIdx_0_flag = snapshots_2_robIdx_0_flag;
  assign io_snapshots_2_robIdx_0_value = snapshots_2_robIdx_0_value;
  assign io_snapshots_2_robIdx_1_flag = snapshots_2_robIdx_1_flag;
  assign io_snapshots_2_robIdx_1_value = snapshots_2_robIdx_1_value;
  assign io_snapshots_2_robIdx_2_flag = snapshots_2_robIdx_2_flag;
  assign io_snapshots_2_robIdx_2_value = snapshots_2_robIdx_2_value;
  assign io_snapshots_2_robIdx_3_flag = snapshots_2_robIdx_3_flag;
  assign io_snapshots_2_robIdx_3_value = snapshots_2_robIdx_3_value;
  assign io_snapshots_2_robIdx_4_flag = snapshots_2_robIdx_4_flag;
  assign io_snapshots_2_robIdx_4_value = snapshots_2_robIdx_4_value;
  assign io_snapshots_2_robIdx_5_flag = snapshots_2_robIdx_5_flag;
  assign io_snapshots_2_robIdx_5_value = snapshots_2_robIdx_5_value;
  assign io_snapshots_2_robIdx_6_flag = snapshots_2_robIdx_6_flag;
  assign io_snapshots_2_robIdx_6_value = snapshots_2_robIdx_6_value;
  assign io_snapshots_2_robIdx_7_flag = snapshots_2_robIdx_7_flag;
  assign io_snapshots_2_robIdx_7_value = snapshots_2_robIdx_7_value;
  assign io_snapshots_2_isCFI_0 = snapshots_2_isCFI_0;
  assign io_snapshots_2_isCFI_1 = snapshots_2_isCFI_1;
  assign io_snapshots_2_isCFI_2 = snapshots_2_isCFI_2;
  assign io_snapshots_2_isCFI_3 = snapshots_2_isCFI_3;
  assign io_snapshots_2_isCFI_4 = snapshots_2_isCFI_4;
  assign io_snapshots_2_isCFI_5 = snapshots_2_isCFI_5;
  assign io_snapshots_2_isCFI_6 = snapshots_2_isCFI_6;
  assign io_snapshots_2_isCFI_7 = snapshots_2_isCFI_7;
  assign io_snapshots_3_robIdx_0_flag = snapshots_3_robIdx_0_flag;
  assign io_snapshots_3_robIdx_0_value = snapshots_3_robIdx_0_value;
  assign io_snapshots_3_robIdx_1_flag = snapshots_3_robIdx_1_flag;
  assign io_snapshots_3_robIdx_1_value = snapshots_3_robIdx_1_value;
  assign io_snapshots_3_robIdx_2_flag = snapshots_3_robIdx_2_flag;
  assign io_snapshots_3_robIdx_2_value = snapshots_3_robIdx_2_value;
  assign io_snapshots_3_robIdx_3_flag = snapshots_3_robIdx_3_flag;
  assign io_snapshots_3_robIdx_3_value = snapshots_3_robIdx_3_value;
  assign io_snapshots_3_robIdx_4_flag = snapshots_3_robIdx_4_flag;
  assign io_snapshots_3_robIdx_4_value = snapshots_3_robIdx_4_value;
  assign io_snapshots_3_robIdx_5_flag = snapshots_3_robIdx_5_flag;
  assign io_snapshots_3_robIdx_5_value = snapshots_3_robIdx_5_value;
  assign io_snapshots_3_robIdx_6_flag = snapshots_3_robIdx_6_flag;
  assign io_snapshots_3_robIdx_6_value = snapshots_3_robIdx_6_value;
  assign io_snapshots_3_robIdx_7_flag = snapshots_3_robIdx_7_flag;
  assign io_snapshots_3_robIdx_7_value = snapshots_3_robIdx_7_value;
  assign io_snapshots_3_isCFI_0 = snapshots_3_isCFI_0;
  assign io_snapshots_3_isCFI_1 = snapshots_3_isCFI_1;
  assign io_snapshots_3_isCFI_2 = snapshots_3_isCFI_2;
  assign io_snapshots_3_isCFI_3 = snapshots_3_isCFI_3;
  assign io_snapshots_3_isCFI_4 = snapshots_3_isCFI_4;
  assign io_snapshots_3_isCFI_5 = snapshots_3_isCFI_5;
  assign io_snapshots_3_isCFI_6 = snapshots_3_isCFI_6;
  assign io_snapshots_3_isCFI_7 = snapshots_3_isCFI_7;
  assign io_enqPtr_flag = snptEnqPtr_flag;
  assign io_enqPtr_value = snptEnqPtr_value;
  assign io_deqPtr_flag = snptDeqPtr_flag;
  assign io_deqPtr_value = snptDeqPtr_value;
  assign io_valids_0 = snptValids_0;
  assign io_valids_1 = snptValids_1;
  assign io_valids_2 = snptValids_2;
  assign io_valids_3 = snptValids_3;
endmodule

