...
module PTWRepeaterNB(
  ...
  reg         sent;
  wire        _delay_io_out;
  wire        _sent_T_probe = ~sent & io_tlb_req_0_valid;
  reg  [37:0] req_vpn;
  reg  [1:0]  req_s2xlate;
  ...
  reg         resp_s2_gpf;
  reg         resp_s2_gaf;
  wire        _recv_T_1_probe = io_tlb_resp_ready & recv;
  wire        _GEN = io_tlb_req_0_valid & sent;
  wire        _GEN_0 = _GEN & io_ptw_req_0_ready;
  wire        _GEN_1 = io_ptw_resp_valid & recv;
  wire        _GEN_2 = _GEN_1 & io_tlb_resp_ready;
  always @(posedge clock) begin
    if (_sent_T_probe) begin
  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      sent <= ~(io_ptw_req_0_ready & sent | _delay_io_out) & (_sent_T_probe | sent);
      recv <= ~(_recv_T_1_probe | _delay_io_out) & (_recv_T | recv);
    end
