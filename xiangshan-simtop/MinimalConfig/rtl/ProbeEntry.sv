...
module ProbeEntry(
  ...
  reg  [1:0]  req_param;
  reg         req_needData;
  wire        _io_block_addr_valid_T_probe = |state;
  ...
  wire        io_pipe_req_valid_0 = _GEN_1 & ~io_pipe_req_valid_REG;
  wire        _GEN_2 =
    _GEN_1 & io_lrsc_locked_block_valid
    & io_lrsc_locked_block_bits[47:6] == req_addr[47:6];
  wire        _GEN_3 = _GEN_1 & io_pipe_req_valid_0 & ~io_pipe_req_ready;
  always @(posedge clock or posedge reset) begin
    if (reset)
      state <= 2'h0;
    else if (state == 2'h2 & io_pipe_resp_valid & io_pipe_resp_bits_id == io_id)
      state <= 2'h0;
    else if (_GEN_1 & io_pipe_req_ready & io_pipe_req_valid_0)
      state <= 2'h2;
    else if (_GEN)
      state <= 2'h1;
