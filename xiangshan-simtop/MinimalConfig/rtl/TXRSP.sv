...
module TXRSP(
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & ~(~io_pipeRsp_valid | io_pipeRsp_bits_txChannel[1])) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: txChannel is wrong for TXRSP\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & _inflightCnt_T_30 > 5'h10) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: in-flight overflow at TXRSP\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
