...
module Decoupled2LCredit(
  ...
  wire         io_in_ready_0;
  wire         acceptLCredit_probe = io_out_lcrdv & (|io_state_state);
  reg  [3:0]   lcreditPool;
  wire         flitv =
    io_in_ready_0 & io_in_valid | ~io_in_valid & (&io_state_state) & (|lcreditPool);
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (acceptLCredit_probe & ~flitv & ~reset & (&lcreditPool)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: L-Credit pool overflow\n");
        if (`STOP_COND_)
