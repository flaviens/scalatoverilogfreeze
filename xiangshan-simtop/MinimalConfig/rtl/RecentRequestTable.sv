...
module RecentRequestTable(
    ...
    always @(posedge clock) begin
      if (~reset & REG) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: single port SRAM should not read and write at the same time\n");
        if (`STOP_COND_)
  ...
  reg  [11:0] s1_hit_REG;
  reg         s2_valid;
  wire        io_w_ready_0 = _rrTable_io_w_req_ready & ~io_r_req_valid;
  reg  [5:0]  io_r_resp_bits_ptr_r;
  reg         io_r_resp_bits_hit_r;
  always @(posedge clock) begin
    REG <= io_w_ready_0 & io_w_valid & io_r_req_valid;
