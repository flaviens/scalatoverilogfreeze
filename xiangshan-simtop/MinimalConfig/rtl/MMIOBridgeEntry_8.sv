...
module MMIOBridgeEntry_8(
  ...
  reg          isRead;
  wire         _GEN = _isRead_T_1 & _isRead_T;
  wire         _GEN_0 = io_req_bits_opcode == 4'h0 | io_req_bits_opcode == 4'h1;
  wire         _GEN_1 = io_chi_rx_dat_ready_0 & io_chi_rx_dat_valid;
  wire         dataCheck =
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (_GEN_1 & ~reset & dataCheck) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: UC should not have DataCheck error\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  `endif // not def SYNTHESIS
  wire         _GEN_2 = io_chi_rx_rsp_ready_0 & io_chi_rx_rsp_valid;
  wire         _GEN_3 = io_chi_rx_rsp_bits_opcode == 5'h5;
  wire         _GEN_4 = _GEN_2 & (_GEN_3 | io_chi_rx_rsp_bits_opcode == 5'h4);
  wire         _GEN_5 =
    _GEN_3 | io_chi_rx_rsp_bits_opcode == 5'h6 | io_chi_rx_rsp_bits_opcode == 5'hE;
  wire         _GEN_6 = _GEN_2 & _GEN_5;
  wire         _GEN_7 = io_chi_rx_rsp_bits_opcode == 5'h3;
  ...
  wire         _io_chi_rx_rsp_ready_T_probe = ~w_comp;
  wire         _io_chi_rx_rsp_ready_T_1_probe = ~w_dbidresp;
  wire         _io_waitOnReadReceipt_T_probe = ~w_readreceipt;
  assign io_chi_rx_rsp_ready_0 = (~w_comp | ~w_dbidresp | ~w_readreceipt) & s_txreq;
  wire         _io_chi_rx_dat_ready_T_probe = ~w_compdata;
  assign io_chi_rx_dat_ready_0 = ~w_compdata & s_txreq;
  wire         _GEN_9 = _isRead_T_1 & _GEN_0;
  wire         _GEN_10 = io_chi_tx_req_valid_0 & ~io_chi_tx_req_ready;
  wire         _GEN_11 = _GEN_8 & isRead;
  wire         _GEN_12 = _GEN_8 & ~isRead;
  wire         _GEN_13 = ~w_pcrdgrant & isRead;
  wire         _GEN_14 = ~w_pcrdgrant & ~isRead;
  wire         _GEN_15 = ~_isRead_T_1 | _isRead_T | ~_GEN_0;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      s_txreq <=
        ~_GEN_8 & (io_chi_tx_req_ready & io_chi_tx_req_valid_0 | ~_isRead_T_1 & s_txreq);
      s_ncbwrdata <= io_chi_tx_dat_ready & io_chi_tx_dat_valid_0 | _GEN_15 & s_ncbwrdata;
      s_resp <= io_resp_ready & io_resp_valid_0 | ~_isRead_T_1 & s_resp;
      w_comp <= _GEN_4 | _GEN_15 & w_comp;
      w_dbidresp <= _GEN_6 | _GEN_15 & w_dbidresp;
      w_compdata <= _GEN_1 | ~_GEN & w_compdata;
      w_pcrdgrant <= io_pCrd_grant | ~_GEN_8 & w_pcrdgrant;
      w_readreceipt <= _GEN_2 & io_chi_rx_rsp_bits_opcode == 5'h8 | ~_GEN & w_readreceipt;
      allowRetry <= ~_GEN_8 & (_isRead_T_1 | allowRetry);
      if (_isRead_T_1)
        isRead <= _isRead_T;
    end
  ...
  always @(posedge clock) begin
    ...
    denied <=
      _GEN_4
        ...
        : _GEN_1 ? denied | (&io_chi_rx_dat_bits_respErr) : ~_isRead_T_1 & denied;
    corrupt <=
      _GEN_1
        ...
        : ~_isRead_T_1 & corrupt;
    traceTag <= _GEN_6 ? io_chi_rx_rsp_bits_traceTag : ~_isRead_T_1 & traceTag;
  end // always @(posedge)
