...
module Sc_Anon_2(
  ...
  wire       trainReadValid = io_trainReadReq_valid & io_trainReadReq_bits_bankMask[0];
  wire       sram_0_io_r_req_valid = predictReadValid | trainReadValid;
  wire       predictReadValid_1 =
    io_predictReadReq_valid & io_predictReadReq_bits_bankMask[1];
  wire       trainReadValid_1 = io_trainReadReq_valid & io_trainReadReq_bits_bankMask[1];
  wire       sram_1_io_r_req_valid = predictReadValid_1 | trainReadValid_1;
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & predictReadValid & trainReadValid) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: read conflict in scglobalTable0_0\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & predictReadValid_1 & trainReadValid_1) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: read conflict in scglobalTable0_1\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
