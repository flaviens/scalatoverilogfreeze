...
module AXI4Flash(
  ...
  wire            DifftestFlash_io_en = nodeIn_ar_ready & nodeIn_ar_valid;
  ...
  wire            nodeIn_r_bits_last =
    readBeatCnt == (DifftestFlash_io_en ? nodeIn_ar_bits_len : len_hold_data);
  `ifndef SYNTHESIS
    wire _GEN = DifftestFlash_io_en & ~reset;
  ...
  wire [3:0][1:0] _GEN_1 =
    {{(&state) & nodeIn_b_ready & nodeIn_b_valid ? 2'h0 : state},
     ...
     {_nodeIn_b_bits_user_T ? 2'h2 : DifftestFlash_io_en ? 2'h1 : state}};
  always @(posedge clock) begin
    if (reset) begin
      state <= 2'h0;
      readBeatCnt <= 8'h0;
  ...
  DifftestFlash DifftestFlash (
    .clock   (clock),
    .io_en   (DifftestFlash_io_en),
    .io_addr
      ({16'h0,
        DifftestFlash_io_en ? nodeIn_ar_bits_addr[15:3] : raddr_hold_data[15:3],
        3'h0}),
