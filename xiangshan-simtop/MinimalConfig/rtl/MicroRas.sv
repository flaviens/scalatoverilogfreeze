...
module MicroRas(
  ...
  input  [3:0]  io_specIn_cfiPosition,
  ...
  input  [3:0]  io_overrideData_bits_cfiPosition,
  ...
  reg         redirectDelay1;
  wire        s3_realPush = io_overrideData_bits_attribute_rasAction == 2'h2;
  wire        s3_realPop = io_overrideData_bits_attribute_rasAction == 2'h1;
  wire        s3_overrideHasRasAction_probe =
    io_overrideData_valid & (s3_realPush | s3_realPop);
  reg         s3_overrideHasRasActionNext;
  wire        _GEN = s3_overrideHasRasActionNext & io_stageCtrl_s1_fire & specPop;
  wire [49:0] _s3_realPushAddr_T_3 =
    50'({46'(io_overrideData_bits_startPc_addr[48:3]
             + {45'h0, io_overrideData_bits_cfiPosition[3]}),
         io_overrideData_bits_cfiPosition[2:0],
         1'h0} + 50'h2);
  wire        specPush = io_specIn_attribute_rasAction == 2'h2;
  wire [49:0] _specPushAddr_T_3 =
    50'({46'(io_specIn_startPc_addr[48:3] + {45'h0, io_specIn_cfiPosition[3]}),
         io_specIn_cfiPosition[2:0],
         1'h0} + 50'h2);
  wire        _GEN_0 = io_overrideData_valid | io_hasRedirect;
  wire        _GEN_1 = io_stageCtrl_s1_fire & specPush;
  wire        _GEN_2 = io_stageCtrl_s1_fire & specPop;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      redirectDelay1 <= 1'h0;
      s3_overrideHasRasActionNext <= 1'h0;
    end
    else begin
      s2_hasPush <=
        ~_GEN_0 & (io_stageCtrl_s1_fire ? specPush : ~io_stageCtrl_s2_fire & s2_hasPush);
      s2_hasPop <=
        ~_GEN_0 & (io_stageCtrl_s1_fire ? specPop : ~io_stageCtrl_s2_fire & s2_hasPop);
      s3_hasPush <=
        ~_GEN_0
        & (io_stageCtrl_s2_fire ? s2_hasPush : ~io_stageCtrl_s3_fire & s3_hasPush);
      s3_hasPop <=
        ~_GEN_0 & (io_stageCtrl_s2_fire ? s2_hasPop : ~io_stageCtrl_s3_fire & s3_hasPop);
      if (_GEN_1)
      ...
      if (io_hasRedirect) begin
        ...
        topRetAddr_addr <=
          s3_realPop
            ? 49'h0
            : s3_realPush ? _s3_realPushAddr_T_3[49:1] : io_fullRetAddr_addr;
      else if (_GEN_1)
      ...
      s3_overrideHasRasActionNext <= s3_overrideHasRasAction_probe;
    end
  ...
  `ifdef ENABLE_INITIAL_REG_
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        s3_overrideHasRasActionNext = _RANDOM[3'h4][25];
      `endif // RANDOMIZE_REG_INIT
      if (reset) begin
        s2_hasPush = 1'h0;
        s2_hasPop = 1'h0;
