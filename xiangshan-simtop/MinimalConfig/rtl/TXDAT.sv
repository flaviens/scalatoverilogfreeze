...
module TXDAT(
  ...
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if (~reset & ~(~io_in_valid | io_in_bits_task_txChannel[2])) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: txChannel is wrong for TXDAT\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & ~(~io_in_valid | _queue_io_enq_ready)) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: TXDAT should never be full\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
      if (~reset & _inflightCnt_T_30 > 5'h10) begin
        if (`ASSERT_VERBOSE_COND_)
          $fwrite(32'h80000002, "Assertion failed: in-flight overflow at TXDAT\n");
        if (`STOP_COND_)
          xs_assert_v2(`__FILE__, `__LINE__);
      end
    end // always @(posedge)
  ...
  assign io_in_ready = _queue_io_enq_ready;
