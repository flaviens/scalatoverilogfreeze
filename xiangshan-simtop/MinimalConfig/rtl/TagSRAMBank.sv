...
module TagSRAMBank(
  ...
  reg  [7:0]  rst_cnt;
  wire [42:0] wdata = rst_cnt[7] ? {io_write_bits_ecc, io_write_bits_tag} : 43'h0;
  wire        wen = ~(rst_cnt[7]) | io_write_valid;
  wire        tag_array_io_r_req_valid_probe = ~wen & io_read_valid;
  ...
  SRAMTemplate_210 tag_array (
    .clock                          (clock),
    .reset                          (reset),
    .io_r_req_valid                 (tag_array_io_r_req_valid_probe),
    ...
    .io_w_req_bits_waymask          (rst_cnt[7] ? io_write_bits_way_en : 2'h3),
